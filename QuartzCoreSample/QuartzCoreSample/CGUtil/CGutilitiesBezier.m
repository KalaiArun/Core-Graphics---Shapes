...
CGMutablePathRef CreateSmoothedBezierPathFromPath( CGPathRef path, CGFloat tention)
{
    ...
    for (int i = 0; i<my_dataPointer.numberOfPoints + (int) my_dataPointer.isClosed; i++) 
    {
        ...
        //NSLog(@"%d -  %f,%f", i, my_dataPointer.indexx[i], my_dataPointer.indexy[i]);
        
        CGAffineTransform trans = CGAffineTransformMakeTranslation(0, 0);
        
        if(i==0)
            CGPathMoveToPoint(bezierPath, &trans, my_dataPointer.indexx[0], my_dataPointer.indexy[0]);
        else if (i==my_dataPointer.numberOfPoints) //this happens when the path is closed
            CGPathAddCurveToPoint(bezierPath, &trans, controlPoints_x[i*2-2], controlPoints_y[i*2-2], controlPoints_x[i*2-1], controlPoints_y[i*2-1],my_dataPointer.indexx[0], my_dataPointer.indexy[0]);
        else
            CGPathAddCurveToPoint(bezierPath, &trans, controlPoints_x[i*2-2], controlPoints_y[i*2-2], controlPoints_x[i*2-1], controlPoints_y[i*2-1],my_dataPointer.indexx[i], my_dataPointer.indexy[i]);
